...
def test_brain_accepts_valid_json():
    brain = Brain(FakeNIM([valid_json()]))
...
def test_brain_retries_then_passes():
    brain = Brain(FakeNIM(["bad", valid_json()]))
...
def test_brain_uses_configured_timeout_and_sampling_params():
    fake = FakeNIM([valid_json()])
    brain = Brain(fake, timeout_seconds=300, reasoning_budget=1234, temperature=0.2)
